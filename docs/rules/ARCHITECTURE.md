...
- `GET /api/signals/open` returns open work signals newest-first; signal extraction, persistence, and resolution stay in `lib/work-signals.ts`. GitHub/Discord syncs persist signals after inserting `raw_activity`; a `completion` signal resolves open signals sharing its task key. The dashboard renders open signals below the digest.
